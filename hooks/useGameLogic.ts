...
export const useGameLogic = () => {
	...
	const applyGravity = (
		board: Board
	): { newBoard: Board; hasChanges: boolean } => {
		...
		(
			...
			return new Promise((resolve) => {
				...
				const processNextCascade = () => {
					// Apply gravity first
					const { newBoard: gravityBoard, hasChanges } =
					...
					matches.forEach((match) => {
						if (gameVariant === 'classic') {
							...
							const kingPosition = findLowestKingPosition(
								workingBoard,
								match.positions
							);
									...
									(conv) =>
										!conversions.some(
											(newConv) =>
												...
												JSON.stringify(newConv.positions) ===
													JSON.stringify(conv.positions)
										)
								)
							);
						}, 1000);
				...
				setTimeout(async () => {
					...
					if (gameVariant === 'classic') {
						...
						if (initialMatch) {
							...
							// Store conversion for animation
							setKingConversions([
								{ positions: initialMatch, player: currentPlayer },
							]);
							...
							// Process cascades
							const { finalBoard, totalCascades } = await processCascades(
								newBoard,
								0,
								onMatchFound
							);
							...
							const { finalBoard } = await processCascades(
								newBoard,
								0,
								onMatchFound
							);
	...
	const setGameModeHandler = useCallback(
		(mode: GameMode) => {
			setGameMode(mode);
			resetGame();
		},
	...
	const setGameVariantHandler = useCallback(
		(variant: GameVariant) => {
