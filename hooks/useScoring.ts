...
import { GameScore, ScoreEvent, CascadeData, Achievement, LevelStats, ScoreAnimation } from '@/types/scoring';
...
export const useScoring = (gameVariant: GameVariant = 'connect3') => {
  ...
  // Award achievement
  const awardAchievement = useCallback((achievementId: string, position?: { x: number; y: number }) => {
    setGameScore(prev => {
  ...
  // Award bonus points
  const awardBonus = useCallback((points: number, description: string, position?: { x: number; y: number }) => {
    const scoreEvent: ScoreEvent = {
      type: 'bonus',
      points,
      multiplier: 1,
