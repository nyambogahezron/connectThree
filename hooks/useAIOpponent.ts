...
export const useAIOpponent = () => {
  ...
  const checkWinningMove = useCallback((board: Board, col: number, player: Player): boolean => {
  ...
  const findLowestEmptyRow = useCallback((board: Board, col: number): number => {
  ...
  const checkWin = useCallback((board: Board, row: number, col: number, player: Player) => {
    const directions = [
      [0, 1],   // horizontal
      [1, 0],   // vertical
      [1, 1],   // diagonal \
  ...
  const evaluateAllMoves = useCallback((board: Board, aiPlayer: Player, opponent: Player): MoveEvaluation[] => {
