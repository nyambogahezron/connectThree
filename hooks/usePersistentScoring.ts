...
import { GameScore, ScoreEvent, Achievement, ScoreAnimation } from '@/types/scoring';
...
export const usePersistentScoring = (config: PersistentScoringConfig) => {
  ...
  const initializeSession = useCallback(async () => {
    try {
      ...
      const achievements = await DatabaseService.getPlayerAchievements(config.playerId);
      
      setGameScore(prev => ({
        ...prev,
        highScore: config.gameVariant === 'connect3' 
  ...
  const calculateBasePoints = useCallback((matchSize: number, isKing: boolean = false): number => {
    let basePoints = 0;
  ...
  // Add score animation
  const addScoreAnimation = useCallback((points: number, position: { x: number; y: number }, color: string = '#4f46e5') => {
  ...
  const awardMatchPoints = useCallback(async (
    matchSize: number,
    ...
    const simultaneousBonus = simultaneousMatches > 1 ? 1 + (simultaneousMatches - 1) * 0.5 : 1;
    
    const totalPoints = Math.round(basePoints * cascadeMultiplier * simultaneousBonus);
    ...
    await DatabaseService.addScoreEvent(sessionId, config.playerId, {
      ...
      description: `${matchSize} ${isKing ? 'King ' : ''}Match${cascadeLevel > 0 ? ` (Cascade x${cascadeLevel + 1})` : ''}`,
      matchSize,
      cascadeLevel,
      isKingMatch: isKing,
      simultaneousMatches,
    ...
    if (position) {
      addScoreAnimation(totalPoints, position, isKing ? '#fbbf24' : '#4f46e5');
    }
  }, [sessionId, config.playerId, calculateBasePoints, getCascadeMultiplier, addScoreAnimation]);
  ...
  const awardAchievement = useCallback(async (
    ...
    const hasAchievement = await DatabaseService.hasAchievement(config.playerId, achievementId);
  ...
  const completeSession = useCallback(async (
    ...
    try {
      ...
      await DatabaseService.updatePlayerStats(config.playerId, config.gameVariant, {
  ...
  }, [sessionId, config.playerId, config.gameVariant, gameScore.currentScore, gameScore.totalMatches, gameScore.currentCascadeLevel]);
  ...
  return {
    ...
    completeSession,
    resetSession,
    incrementMoves: () => { moveCounterRef.current++; },
  };
};