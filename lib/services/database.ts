...
export class DatabaseService {
  ...
  static async updatePlayer(id: string, updates: Partial<Pick<Player, 'name' | 'avatar'>>): Promise<Player | null> {
  ...
  static async createGameSession(
    ...
    const [session] = await db.insert(gameSessions).values(newSession).returning();
    return session;
  }
  ...
  static async getPlayerGameHistory(playerId: string, limit = 20): Promise<GameSession[]> {
    return await db
      .select()
      .from(gameSessions)
  ...
  static async getPlayerScoreForSession(sessionId: string, playerId: string): Promise<PlayerScore | null> {
  ...
  static async getPlayerScoreEvents(playerId: string, limit = 50): Promise<any[]> {
    return await db
      .select()
      .from(scoreEvents)
  ...
  // Player statistics
  static async updatePlayerStats(playerId: string, gameVariant: GameVariant, gameResult: {
    won: boolean;
    draw: boolean;
    score: number;
    matches: number;
    ...
    const updates: Partial<PlayerStats> = {
      ...
      maxCascadeInGame: Math.max(stats?.maxCascadeInGame || 0, gameResult.maxCascadeLevel),
      totalKingsCreated: (stats?.totalKingsCreated || 0) + gameResult.kingsCreated,
      perfectGames: (stats?.perfectGames || 0) + (gameResult.perfectGame ? 1 : 0),
    ...
    const connect3Games = isConnect3 ? updates.connect3GamesPlayed! : (stats?.connect3GamesPlayed || 0);
    const classicGames = isClassic ? updates.classicGamesPlayed! : (stats?.classicGamesPlayed || 0);
    ...
    if (connect3Games > 0) {
      updates.connect3WinRate = (updates.connect3Wins || stats?.connect3Wins || 0) / connect3Games;
      updates.connect3AverageScore = (updates.connect3TotalScore || stats?.connect3TotalScore || 0) / connect3Games;
    }

    if (classicGames > 0) {
      updates.classicWinRate = (updates.classicWins || stats?.classicWins || 0) / classicGames;
      updates.classicAverageScore = (updates.classicTotalScore || stats?.classicTotalScore || 0) / classicGames;
    }
  ...
  private static async updateGameVariantLeaderboard(variant: 'connect3' | 'classic'): Promise<void> {
    // Clear existing leaderboard for this variant
    await db.delete(leaderboards).where(eq(leaderboards.category, variant));

    const scoreField = variant === 'connect3' ? playerStats.connect3TotalScore : playerStats.classicTotalScore;
    const gamesField = variant === 'connect3' ? playerStats.connect3GamesPlayed : playerStats.classicGamesPlayed;
    const winsField = variant === 'connect3' ? playerStats.connect3Wins : playerStats.classicWins;
  ...
  static async getPlayerInsights(playerId: string): Promise<any> {
    ...
    return {
      ...
      achievements,
      insights: {
        favoriteGameMode: stats?.connect3GamesPlayed > stats?.classicGamesPlayed ? 'connect3' : 'classic',
        averageGameDuration: stats?.totalGamesPlayed > 0 ? stats.totalPlayTime / stats.totalGamesPlayed : 0,
        matchesPerGame: stats?.totalGamesPlayed > 0 ? stats.totalMatches / stats.totalGamesPlayed : 0,
        cascadeEfficiency: stats?.totalMatches > 0 ? stats.totalCascades / stats.totalMatches : 0,
      },
    };
  }
}